    ...
    <taon-admin-mode-configuration>
      @if (itemsLoaded()) {
        ...
        <footer
          ...
          Copyright <strong>inject-tests</strong> {{ year }}
        </footer>
      }
    </taon-admin-mode-configuration>
  `,
})
export class InjectTestsApp implements OnInit {
  ...
  navItems =
    InjectTestsClientRoutes.length <= 1
      ? []
      : InjectTestsClientRoutes.filter(r => r.path !== undefined).map(r => ({
          path: r.path === '' ? '/' : `/${r.path}`,
...
//#region  inject-tests api service
...
//#region  inject-tests routes

//#region @browser
export const InjectTestsServerRoutes: ServerRoute[] = [
  {
    path: '**',
    renderMode: RenderMode.Prerender,
  },
];
export const InjectTestsClientRoutes: Routes = [
  {
    path: '',
    pathMatch: 'full',
    redirectTo: () => {
      if (InjectTestsClientRoutes.length === 1) {
        return '';
      }
      return InjectTestsClientRoutes.find(r => r.path !== '')!.path!;
    },
  },
  // PUT ALL ROUTES HERE
...
//#region  inject-tests app configs

//#region @browser
export const InjectTestsAppConfig: ApplicationConfig = {
  providers: [
    provideZonelessChangeDetection(),
    {
    ...
    }),
    {
      ...
      multi: true,
      useFactory: () => InjectTestsStartFunction,
    },
    provideBrowserGlobalErrorListeners(),
    // remove withHashLocation() to use SSR
    provideRouter(
      InjectTestsClientRoutes,
      withHashLocation(),
      withComponentInputBinding(),
    ),
...
export const InjectTestsServerConfig: ApplicationConfig = {
  providers: [provideServerRendering(withRoutes(InjectTestsServerRoutes))],
};

export const InjectTestsConfig = mergeApplicationConfig(
  InjectTestsAppConfig,
  InjectTestsServerConfig,
);
//#endregion

//#endregion

//#region  inject-tests start function
export const InjectTestsStartFunction = async (
  ...
  const sessionController = AppContext.getClassInstance(SessionController);
...
export default InjectTestsStartFunction;
//#endregion
