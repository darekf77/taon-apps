...
//#region backend-backend-realtime component
//#region @browser
@Component({
  selector: 'app-root',
    ...
    <taon-admin-mode-configuration>
      @if (itemsLoaded()) {
        ...
        @if (navItems.length === 0 || forceShowBaseRootApp) {
          <mat-card class="m-2">
            <mat-card-content>
              ...
              Name: backend-backend-realtime<br />
        ...
        <footer
          ...
          Copyright <strong>backend-backend-realtime</strong> {{ year }}
        </footer>
      }
    </taon-admin-mode-configuration>
  `,
})
export class BackendBackendRealtimeApp implements OnInit {
  ...
  navItems =
    BackendBackendRealtimeClientRoutes.length <= 1
      ? []
      : BackendBackendRealtimeClientRoutes.filter(
          r => r.path !== undefined,
        ).map(r => ({
          path: r.path === '' ? '/' : `/${r.path}`,
...
//#region  backend-backend-realtime api service
...
//#region  backend-backend-realtime routes
//#region @browser
export const BackendBackendRealtimeServerRoutes: ServerRoute[] = [
  {
    path: '**',
    renderMode: RenderMode.Prerender,
  },
];
export const BackendBackendRealtimeClientRoutes: Routes = [
  {
    path: '',
    pathMatch: 'full',
    redirectTo: () => {
      if (BackendBackendRealtimeClientRoutes.length === 1) {
        return '';
      }
      return BackendBackendRealtimeClientRoutes.find(r => r.path !== '')!.path!;
    },
  },
  // PUT ALL ROUTES HERE
...
//#region  backend-backend-realtime app configs
//#region @browser
export const BackendBackendRealtimeAppConfig: ApplicationConfig = {
  providers: [
    provideZonelessChangeDetection(),
    {
      provide: TAON_CONTEXT,
      useFactory: () => BackendBackendRealtimeContext,
    },
    providePrimeNG({
      theme: {
        preset: Aura,
    ...
    }),
    {
      ...
      multi: true,
      useFactory: () => BackendBackendRealtimeStartFunction,
    },
    provideBrowserGlobalErrorListeners(),
    // remove withHashLocation() to use SSR
    provideRouter(
      BackendBackendRealtimeClientRoutes,
      withHashLocation(),
      withComponentInputBinding(),
    ),
...
export const BackendBackendRealtimeServerConfig: ApplicationConfig = {
  providers: [
    provideServerRendering(withRoutes(BackendBackendRealtimeServerRoutes)),
  ],
};

export const BackendBackendRealtimeConfig = mergeApplicationConfig(
  BackendBackendRealtimeAppConfig,
  BackendBackendRealtimeServerConfig,
);
//#endregion
//#endregion

//#region  backend-backend-realtime entity
...
//#region  backend-backend-realtime controller
...
//#region  backend-backend-realtime migration
...
//#region  backend-backend-realtime context
var BackendBackendRealtimeContext = Taon.createContext(() => ({
  ...HOST_CONFIG['BackendBackendRealtimeContext'],
...
//#region  backend-backend-realtime start function
export const BackendBackendRealtimeStartFunction = async (
  ...
  await BackendBackendRealtimeContext.initialize();
...
export default BackendBackendRealtimeStartFunction;
//#endregion
