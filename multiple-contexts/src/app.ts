...
//#region multiple-contexts component
...
export class MultipleContextsApp {
  itemsLoaded = signal(false);
  navItems =
    MultipleContextsClientRoutes.length <= 1
      ? []
      : MultipleContextsClientRoutes.filter(r => r.path !== undefined).map(
          r => ({
            path: r.path === '' ? '/' : `/${r.path}`,
...
//#region  multiple-contexts api service
...
//#region  multiple-contexts routes
//#region @browser
export const MultipleContextsServerRoutes: ServerRoute[] = [
  {
    path: '**',
    renderMode: RenderMode.Prerender,
  },
];
export const MultipleContextsClientRoutes: Routes = [
  {
    path: '',
    pathMatch: 'full',
    redirectTo: () => {
      if (MultipleContextsClientRoutes.length === 1) {
        return '';
      }
      return MultipleContextsClientRoutes.find(r => r.path !== '')!.path!;
    },
  },
  // PUT ALL ROUTES HERE
...
//#region  multiple-contexts app configs
//#region @browser
export const MultipleContextsAppConfig: ApplicationConfig = {
  providers: [
    provideZonelessChangeDetection(),
    {
      provide: TAON_CONTEXT,
      useFactory: () => MultipleContextsContext,
    },
    providePrimeNG({
      theme: {
        preset: Aura,
    ...
    }),
    {
      ...
      multi: true,
      useFactory: () => MultipleContextsStartFunction,
    },
    provideBrowserGlobalErrorListeners(),
    provideRouter(MultipleContextsClientRoutes),
...
export const MultipleContextsServerConfig: ApplicationConfig = {
  providers: [provideServerRendering(withRoutes(MultipleContextsServerRoutes))],
};

export const MultipleContextsConfig = mergeApplicationConfig(
  MultipleContextsAppConfig,
  MultipleContextsServerConfig,
);
//#endregion
//#endregion

//#region  multiple-contexts entity
...
//#region  multiple-contexts controller
...
//#region  multiple-contexts migration
...
//#region  multiple-contexts context
var MultipleContextsContext = Taon.createContext(() => ({
  ...HOST_CONFIG['MultipleContextsContext'],
...
//#region  multiple-contexts start function
const MultipleContextsStartFunction = async (
  startParams?: Taon.StartParams,
): Promise<void> => {
  await MultipleContextsContext.initialize();
...
export default MultipleContextsStartFunction;
